...
Nel layer `CURATED` il `ROLE_DATA_ENGINEER` deve poter effettuare trasformazioni e pubblicazioni (tipicamente tramite `INSERT` e operazioni di upsert/merge). Il `ROLE_DATA_ANALYST` accede in sola lettura per verifiche o analisi intermedie sulla qualità dei dati (inclusa la revisione delle anomalie nelle tabelle di quarantena).
	...
	-- Data Engineer: trasformazioni, caricamenti e gestione quarantena (R/W) inclusa TRUNCATE
	GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON ALL TABLES IN SCHEMA HEALTHCARE_DW.CURATED TO ROLE ROLE_DATA_ENGINEER;
	GRANT SELECT, INSERT, UPDATE, DELETE, TRUNCATE ON FUTURE TABLES IN SCHEMA HEALTHCARE_DW.CURATED TO ROLE ROLE_DATA_ENGINEER;
...
Trattandosi di uno schema puramente tecnico e operativo, l'accesso è strettamente riservato al Data Engineer, con visibilità di audit per il Compliance Officer.
  ...
  GRANT USAGE ON ALL PROCEDURES IN SCHEMA HEALTHCARE_DW.PIPELINE TO ROLE ROLE_DATA_ENGINEER;
  GRANT USAGE ON FUTURE PROCEDURES IN SCHEMA HEALTHCARE_DW.PIPELINE TO ROLE ROLE_DATA_ENGINEER;
